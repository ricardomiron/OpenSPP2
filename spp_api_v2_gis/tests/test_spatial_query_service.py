...
class TestSpatialQueryCountSuppression(TransactionCase):
    ...
    def test_suppress_count_band_including_zero(self):
        """Any count in [0, k) -> (0, True); k or more passes through."""
        service = self._service()
        k = 5
        self.assertEqual(service._suppress_count(0, k), (0, True))
        self.assertEqual(service._suppress_count(1, k), (0, True))
        self.assertEqual(service._suppress_count(4, k), (0, True))
        self.assertEqual(service._suppress_count(5, k), (5, False))
        self.assertEqual(service._suppress_count(123, k), (123, False))
    ...
    def test_query_statistics_suppresses_small_result(self):
        """query_statistics floors a below-threshold count to 0 with the flag set."""
    ...
    def test_batch_suppresses_items_and_summary(self):
        ...
        self.assertTrue(result["results"][0]["count_suppressed"])
        self.assertEqual(result["results"][0]["total_count"], 0)

    def test_proximity_suppresses_small_result(self):
        """query_proximity floors a below-threshold count to 0 with the flag set."""
